...
export class NewsItem extends Component {
  render() {
    return (
      <div
        className="card rounded-0 mx-1"
        style={{ ...this.props.bdStyle, ...this.props.fnStyle3 }}
      >
        <span
        ...
        <img
          ...
          className="card-img-top rounded-0 border-bottom border-black"
          alt="Not Found"
          style={{ height: "200px" }}
        />
        <div className="card-body">
          ...
          >
            <h5 className="card-title" style={{ fontWeight: "bold" }}>
              {this.props.title.length < 55
                ? this.props.title
                : `${this.props.title} ...`}
            </h5>
            <p className="card-text">
              {" "}
              {this.props.desc.length < 120
                ? this.props.desc
                : `${this.props.desc} ...`}
            </p>
          </div>
          <div className="d-flex justify-content-between mt-3 mb-1 mx-2">
            <div>
              <small className="text-body-secondary">
                {new Date(this.props.pubDt).toUTCString()}
              </small>
            </div>
            <a
